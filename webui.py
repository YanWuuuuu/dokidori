...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column(scale=0, min_width=500):
            ...
            with gr.Row():
                ...
                tomoe_btn = gr.Button("巴", variant="secondary", scale=0, min_width=82)
        ...
        with gr.Column():
            with gr.Row():
                gr.Markdown("""
                            ...
                            """)
            with gr.Row():
                staff_btn = gr.Button("staff-san", variant="secondary")
                others_btn = gr.Button("others", variant="secondary")
    ...
    with gr.Row():    
        ...
        num_beams = gr.Slider(minimum=1, maximum=10, step=1, value=5, label="num_beams")
    ...
    tomoe_btn.click(lambda text: text + "<tomoe> ", inputs=input_text, outputs=input_text)
    ...
    staff_btn.click(lambda text: text + "<staff> ", inputs=input_text, outputs=input_text)
    others_btn.click(lambda text: text + "<others> ", inputs=input_text, outputs=input_text)
    
    translate_button.click(fn=translate_multiline, inputs=[input_text, max_new_tokens_slider, batch_size, repetition_penalty, num_beams, temperature_slider, top_p_slider], outputs=output_text)
